...
## Backend-ready-but-UI-missing — 2 endpoint group พบระหว่าง dead-endpoint audit (2026-09-19)

**Audit**: เทียบทุก route decorator (`@Get`/`@Post`/`@Patch`/`@Put`/`@Delete`) ในทุก controller (136 routes) กับทุก path string ที่ปรากฏใน `apps/frontend/src/lib/api/*.ts` และทุกไฟล์ page/component พบ 16 route ที่ไม่มี frontend เรียกเลย — ส่วนใหญ่เป็น pattern ที่รู้จักอยู่แล้ว (invitation flow ที่เลิกใช้แล้ว, assessment evidence ที่ตารางว่าง) แต่ 2 กลุ่มนี้เป็นเคสใหม่ที่ต้องแยกวิเคราะห์เพราะมีทั้งกรณี "dead code ที่ลบได้จริง" (`GET /courses/my-courses` — ลบแล้ว, `c79c9e4 refactor: remove dead GET /courses/my-courses endpoint`) และกรณีนี้ที่เป็น "backend ทำไว้แล้ว แต่ไม่เคยมี UI เรียกเลย":

**`CloPloMappingController` (`/clo-plo-mappings`) — CRUD เต็มรูปแบบ ไม่มี UI ใดๆ เรียกเลยแม้แต่จุดเดียว**: ตรวจแล้วว่า **ไม่มีหน้า ADMIN สำหรับจัดการ Course/CLO/PLO/CloPloMapping เลยทั้งระบบ** (ต่างจาก `CourseCategory`/`CurriculumRequirement`/`Prerequisite` ที่มี UI ใน `/staff/curriculum` แล้ว) แม้แต่ route `GET /clo-plo-mappings` ที่เปิดให้ `STUDENT` อ่านได้ด้วยก็ไม่มีใครเรียก — หน้า `/clo-plo-analysis` (STUDENT) อ่าน PLO achievement ที่คำนวณจาก mapping ไปแล้วผ่าน `fetchStudentPloAchievement`/`fetchPlos`/`fetchCurriculum` แทนที่จะอ่าน mapping ดิบ จึงไม่เคยแตะ endpoint นี้เลย **ไม่ใช่ dead code ที่ควรลบ** — เป็น backend ที่พร้อมสำหรับ ADMIN UI จัดการ Course→CLO→PLO mapping ที่ยังไม่เคยถูกสร้าง (ตอนนี้ข้อมูล CLO/PLO/CloPloMapping ทั้งหมดเข้าระบบผ่าน `seed-plo-clo.ts` เท่านั้น — ดูหัวข้อ "PLO/CLO/CloPloMapping — sample data" ด้านล่าง)

**`GET /course-assessments/course/:courseId/aggregate` — ไม่มี UI เรียกตรง แต่ไม่ใช่ dead code เหมือน `my-courses`**: ตรวจสอบตอนแรกเข้าใจผิดว่าซ้ำกับ `GET /dashboard/instructor` (ซึ่งเรียก `getAggregateForCourse()` ตัวเดียวกันภายในอยู่แล้ว และข้อมูลถูก render จริงใน `instructor/clo-achievement-section.tsx`) — แต่ต่างจาก `my-courses` ตรงที่ **`/dashboard/instructor` เป็น `@Roles('INSTRUCTOR')` เท่านั้นและ scope ด้วย `user.userId` เอง** ส่วน `/course-assessments/course/:courseId/aggregate` เปิดให้ `SUPER_ADMIN`/`ADMIN`/`INSTRUCTOR` ผ่าน `InstructorOrScopeGuard` — ADMIN/SUPER_ADMIN **ไม่มีทางเรียก `/dashboard/instructor` ได้เลย** (role guard บล็อก) จึง endpoint นี้เป็นทางเดียวที่ ADMIN/SUPER_ADMIN จะดู aggregate course assessment ของวิชาใดวิชาหนึ่งได้ แต่ไม่เคยมี UI ฝั่ง ADMIN เรียกเลย — INSTRUCTOR ไม่ต้องการ endpoint นี้เพิ่ม (เห็นผ่าน dashboard ของตัวเองอยู่แล้ว) แต่ถ้าจะทำ ADMIN course drill-down ในอนาคต endpoint นี้พร้อมใช้แล้ว
...
~~**พบเพิ่มระหว่างสำรวจ (ยังไม่แก้ นอกขอบเขตรอบนี้)**: pattern `isActive` + `@@unique` แบบเดียวกันเป๊ะๆ มีอยู่ใน 9 model อื่นด้วย — `Faculty.code`, `Department`, `Program`, `Curriculum`, `CourseCategory`, `Course`, `Plo`, `Clo`, `Semester` — ทุกตัวมีบั๊กแบบเดียวกันในทางทฤษฎี (soft-delete แล้ว re-create ด้วยค่าเดิมจะชน 409) แต่เป็น org-structure ADMIN-only CRUD ที่ soft-delete+re-create ซ้ำชื่อเดิมเป็น edge case ที่เกิดน้อยกว่ามาก ไม่มีใคร flag ไว้มาก่อน ถ้าจะแก้ต้องเป็นงานแยกทีละ scope**~~ — **แก้แล้ว (2026-08-27)**

**แก้แล้ว**: migration `20260827083737_add_soft_delete_partial_unique_indexes` เปลี่ยน `@@unique`/`@unique` เดิมของทั้ง 9 model เป็น partial unique index (`WHERE "isActive" = true`) แบบเดียวกับ `StudentCourseRecord` — เก็บ `Curriculum.@@unique([programId, id])` และ `CourseCategory.@@unique([curriculumId, id])` ไว้เหมือนเดิม (เป็น FK-support constraint สำหรับ compound relation ของ `StudentProfile`/`Course` ไม่เกี่ยวกับ natural-key uniqueness) **ต่างจาก StudentCourseRecord ตรงที่ทั้ง 9 model นี้มี app-level pre-check (`assertXAvailable`) ที่ query ด้วย natural key ตรงๆ โดยไม่กรอง `isActive`** จึงต้องแก้ทั้งสองชั้น: เปลี่ยน `assertXAvailable` ทั้ง 10 จุด (Program มี 2) จาก `findUnique` (compound key) เป็น `findFirst({ where: { ...fields, isActive: true } })` — TypeScript compile error หลัง `prisma generate` ใหม่ (compound-key input type หายไปเพราะ schema เปลี่ยน) ช่วยยืนยันว่าแก้ครบทุกจุดจริง ทดสอบ reproduce บั๊กจริงก่อนแก้กับ 3 ตัวแทน (Faculty, Curriculum, Course — ครอบคลุม single-field/compound-with-FK-support-sibling/compound-FK-relation) แล้วยืนยันหลังแก้ว่าทั้ง 9 model สำเร็จ (201) พร้อมยืนยันว่า duplicate จริงยังโดนบล็อกเหมือนเดิม (409) เพิ่มบันทึก pattern นี้ใน `CONVENTIONS.md` §7 เป็นครั้งแรก
